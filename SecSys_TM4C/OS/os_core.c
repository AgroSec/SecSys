...
int OS_AddThreads(void(*thread0)(void), uint32_t p0,
  ...
  status = StartCritical(); //Disable Interrupts
	...
	*/
	tcbs[2].next = &tcbs[3];	//main thread 7 points to main thread 8
	tcbs[3].next = &tcbs[4];	//main thread 8 points to main thread 9
	tcbs[4].next = &tcbs[0];	//main thread 9 points to main thread 0
	...
	*/
	...
	SetInitialStack(1);	//SetInitialStack initial stack of main thread 8
	Stacks[1][STACKSIZE-2] = (int32_t)(thread8);	//Set address of thread 8 as PC
	SetInitialStack(2);	//SetInitialStack initial stack of main thread 9
	Stacks[2][STACKSIZE-2] = (int32_t)(thread9);	//Set address of thread 9 as PC
