...
int16_t OWSearch()
{
	...
	if (!LastDeviceFlag)
	{
		...
		//OWWriteByte(0xF0);
		uint8_t bitMask;
		GPIO_InitPortOutput(OW_port1, OW_pin1);	// MiSU: optimize for speed
		for (bitMask = 0x01; bitMask; bitMask <<= 1) 
		{
			OWWriteBit( (bitMask & 0xF0)?1:0);
		}
		GPIO_SetPin(OW_port1, OW_pin1, 0);				// drive output low
		GPIO_InitPortInput(OW_port1, OW_pin1, GPIO_PIN_TYPE_OD);	// MiSU: optimize for speed
		
		// loop to do the search
		do
		{
			// read a bit and its complement
			GPIO_InitPortOutput(OW_port1, OW_pin1);	// init output takes about 10us
			id_bit = OWReadBit();
			//GPIO_InitPortOutput(OW_port1, OW_pin1);	// init output takes about 10us
...
void OWWriteBit(uint8_t bit_value)
{
	if (bit_value & 1) {
		// bit is 1
		//DisableInterrupts();
		//GPIO_InitPortOutput(OW_port1, OW_pin1);	// MiSU: optimized for speed
		GPIO_SetPin(OW_port1, OW_pin1, 0);				// drive output low
		delayMicroseconds(6);		//default 6
		GPIO_SetPin(OW_port1, OW_pin1, OW_pin1);	// drive output high		
		//EnableInterrupts();
		//delayMicroseconds(62);	//default 64
		...
		//GPIO_InitPortOutput(OW_port1, OW_pin1);	// MiSU: optimized for speed
		GPIO_SetPin(OW_port1, OW_pin1, 0);				// drive output low
		delayMicroseconds(59);	// default 60
		GPIO_SetPin(OW_port1, OW_pin1, OW_pin1);	// drive output high
		//EnableInterrupts();
